...
def get_aiagents(db: db_dependency):
    return db.query(AIAgent).all()

@router.get('/{aiagent_id}', response_model=AIAgentResponse)
def get_aiagent(db: db_dependency, aiagent_id: int):
    db_aiagent = db.query(AIAgent).filter(AIAgent.id == aiagent_id).first()
...
def create_aiagent(db: db_dependency, aiagent: AIAgentCreate):
    db_aiagent = AIAgent(**aiagent.dict())
    db.add(db_aiagent)
    db.commit()
...
def delete_aiagent(db: db_dependency, aiagent_id: int):
    db_aiagent = db.query(AIAgent).filter(AIAgent.id == aiagent_id).first()
    if db_aiagent is None:
        raise HTTPException(status_code=404, detail="AIAgent not found")
